...
## Made robust for ANY extra required column, not just "team"

Instead of hardcoding a fix for the specific `team` column, both
`admin_add_player` and `ensure_self_player` now scan `players` for
*any* column that's required (`NOT NULL`, no default) and isn't one this
app already knows how to fill (`display_name`, `attend`, `group_name`,
`user_id`). Anything else it finds — `team`, `username`, or anything
else left over on this table — gets a safe blank value automatically
(empty string for text columns, `0` for numbers, `false` for booleans,
etc.).

This means: **admin adding a player, and any user's first login, will
never again be blocked by a legacy column neither of us knew existed.**
No more one-off patches for each newly discovered column.

Re-run the latest `supabase-setup.sql` once more to pick this up.

## Add player now only asks for a name, and guests get their own list

- **Adding a player only requires a name** (plus an optional group).
  Nothing else — username, password, or any other field some other
  tables might expect — is ever asked for. Any such column on the
  database side is auto-filled blank (see the section above); the admin
  never sees or needs to touch it.
- **The roster is now split into two sections:**
  - **Players** — anyone who has signed in with an account, shown under
    their own signed-in display name
  - **Guests** — anyone the admin added manually, with no account
    required at all
- The "Add guest player" box (admin only) is explicitly labeled as
  adding a guest, since that's its only purpose now — signed-in users
  are added to the roster automatically the moment they log in.

Re-run the latest `supabase-setup.sql` if you haven't already, then
refresh the app to see the split roster.

## Actual root cause found: a column with a default of NULL

Your printed function definition confirmed the fix was installed
correctly — yet it still failed. That narrowed it down precisely: the
`team` column apparently has its default explicitly set to `NULL` (not
"no default at all"). My auto-fill logic was checking
`column_default is null` to decide "does this column need a value from
me?", but a default of NULL still counts as *having* a default in
Postgres's system catalogs — so the check skipped `team`, trusting its
default to apply, and that default is exactly the NULL the NOT NULL
constraint rejects.

**The fix:** stop checking for a default at all, and just explicitly
supply a value for every required column not already known — regardless
of whether it has some default or not. An explicit value is always safe.

Both `admin_add_player` and `ensure_self_player` are updated this way in
`supabase-setup.sql`. If you'd rather not re-run the whole script again,
`fix-add-player-only.sql` has just this one corrected function — run
either one, then try "Add player" again.

## Players now appear the instant their account is created

Previously, a player's roster row was only created the first time they
actually logged into the app (via `ensure_self_player`). If your Supabase
project requires email confirmation, someone who just signed up but
hasn't confirmed/logged in yet wouldn't show up in the roster at all
until they did.

A new trigger (`on_auth_user_created`) on `auth.users` now creates the
roster row the moment an account is created — completely independent of
whether the app ever runs. This uses the same safe-blank-value auto-fill
as the other functions, so it can't be blocked by any legacy column
either.

If admin's own signup happens to hit this trigger before the app sets
their name to "ARUN", `ensure_self_player` now also corrects the display
name to match on next login, so it won't get stuck showing an
email-derived name instead.

**Re-run the latest `supabase-setup.sql`** to add this trigger.

## Back to one unified Players list

The roster is one list again instead of separate "Players" and "Guests"
sections. Anyone admin adds manually (no account needed — just a name)
shows up right alongside signed-in users, with a small "Guest" tag next
to their name so it's still easy to tell them apart at a glance. The
add-player box is now just labeled "Add player" since it's the one and
only way to add someone to the roster who isn't signing in themselves.

To be clear: **this has never required signing up or logging in** — the
`user_id` column has always been nullable for exactly this reason. The
only thing that changed here is how it's displayed.
